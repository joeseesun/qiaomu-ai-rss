...
const nativeDrop=new DataTransfer();nativeDrop.setData('application/x-qiaomu-rss-image',dt.getData('application/x-qiaomu-rss-image'));
const cm=leaf.view.containerEl.querySelector('.cm-content'),rect=cm.getBoundingClientRect();
cm.dispatchEvent(new DragEvent('drop',{bubbles:true,cancelable:true,dataTransfer:nativeDrop,clientX:rect.left+40,clientY:rect.top+30}));
...
check('Note embeds local attachment: '+destination,/!\[/.test(leaf.view.editor.getValue())&&leaf.view.editor.getValue().includes(files[0].basename)&&!leaf.view.editor.getValue().includes('blob:'));
}
await leaf.view.save();
await leaf.setViewState({type:'markdown',state:{file:note.path,mode:'preview'}});
let rendered;for(let i=0;i<60;i++){rendered=[...leaf.view.containerEl.querySelectorAll('.markdown-preview-view img')];if(rendered.length===3&&rendered.every(img=>img.naturalWidth>0))break;await new Promise(r=>setTimeout(r,100));}
check('All three attachments visibly render after reopening note',rendered.length===3&&rendered.every(img=>img.naturalWidth>0));
