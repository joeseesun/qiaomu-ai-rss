...
record('RSSHub stays separate from the ten curated feeds',run(`const q=document.querySelector('.qrs-discovery-search');q.value='';q.dispatchEvent(new Event('input'));document.querySelectorAll('.qrs-discovery-collections button')[2].click();return JSON.stringify(document.querySelectorAll('.qrs-discovery-card').length===2&&!document.querySelector('[data-feed="ruanyifeng"]'));`));
...
record('Subscription manager embeds discovery in its Explore tab',run(`document.querySelector('.qrs-subscription-modal [data-tab=explore]').click();${delay};return JSON.stringify(!!document.querySelector('.qrs-subscription-modal .qrs-discovery-card')&&app.workspace.getLeavesOfType('qiaomu-ai-rss-discovery').length===0);`));
