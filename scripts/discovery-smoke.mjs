...
record('Discovery opens nine direct curated feeds without requests',run(`const p=${plugin},transport=p.subscriptions.transport;let requests=0;p.subscriptions.transport=async()=>{requests++;throw Error('Unexpected request');};try{p.center?.close();await p.openDiscovery();return JSON.stringify(requests===0&&document.querySelectorAll('.qrs-discovery-card').length===9&&document.querySelector('.qrs-discovery-standard').textContent.includes('宁缺毋滥')&&!document.querySelector('[data-feed="qiaomu"]'));}finally{p.subscriptions.transport=transport;}`));
