...
record('Discovery opens ten direct curated feeds without requests',run(`const p=${plugin},transport=p.subscriptions.transport;let requests=0;p.subscriptions.transport=async()=>{requests++;throw Error('Unexpected request');};try{for(const leaf of app.workspace.getLeavesOfType('qiaomu-ai-rss-discovery'))leaf.detach();await p.openDiscovery();return JSON.stringify(requests===0&&document.querySelectorAll('.qrs-discovery-card').length===10&&document.querySelector('.qrs-discovery-standard').textContent.includes('宁缺毋滥')&&!document.querySelector('[data-feed="qiaomu"]'));}finally{p.subscriptions.transport=transport;}`));
