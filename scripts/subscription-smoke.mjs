...
const urls = ['https://blog.qiaomu.ai/feed.xml', 'https://simonwillison.net/atom/everything/'];
...
record('Duplicate source rejected through UI', run(`const form=document.querySelector('.qrs-subscription-add');form.querySelector('[type=url]').value=${JSON.stringify(urls[0])};form.requestSubmit();for(let i=0;i<110&&form.querySelector('button').disabled;i++){${delay};}return JSON.stringify(document.querySelector('.qrs-subscription-message').textContent.includes('已经添加')&&${state}.subscriptions.length===1);`));
...
record('Edit name and group through UI', run(`document.querySelector('.qrs-subscription-row [aria-label^="编辑"]').click(); const modal=[...document.querySelectorAll('.modal')].at(-1);const inputs=modal.querySelectorAll('input');inputs[0].value='个人订阅验收';inputs[0].dispatchEvent(new Event('input',{bubbles:true}));inputs[1].value='研究';inputs[1].dispatchEvent(new Event('input',{bubbles:true}));modal.querySelector('button.mod-cta').click();${delay};return JSON.stringify(${state}.subscriptions.some(f=>f.name==='个人订阅验收'&&f.group==='研究'));`));
...
record('Personal articles open in original mode without Qiaomu API calls', run(`const p=${plugin};const api=p.api;p.api=()=>{throw Error('Personal data must not request Qiaomu');};try{document.querySelector('.qrs-entry').click();${delay};return JSON.stringify(document.querySelector('[aria-label="阅读版本"]').value==='original'&&document.querySelector('[aria-label="阅读版本"]').options.length===1&&(document.querySelector('.qrs-prose')?.textContent.length||0)>20);}finally{p.api=api;}`));
record('Favorite and note export work for personal articles', run(`const button=document.querySelector('[aria-label="收藏文章"]');if(button)button.click();${delay};const b=${view}.bundle;const f=await ${plugin}.saveArticle(b,'original',document);return JSON.stringify(!!${state}.favorites[b.entry.id]&&(await app.vault.read(f)).includes('reading_mode: "原文"'));`));
